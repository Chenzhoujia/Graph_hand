...
def create_loss_ops(target_op,input_ops, output_ops):
  ...
  loss_ops = [
      tf.reduce_mean(
          tf.reduce_sum((input_ops.nodes + output_op.nodes - target_op.nodes)**2, axis=-1))
...
def figure_joint_skeleton(uvd_pt1,uvd_pt2,uvd_pt3,test_num):
    ...
    plt.savefig("/home/chen/Documents/GNN_demo/saved/image/"+"/"+str(test_num).zfill(10)+".png")
...
# test data
xyz_test_list = np.loadtxt("/home/chen/Documents/denseReg-master/exp/train_cache/saved/GNN/xyz_all.txt")
#for i in range(8252):
test_num_total = 2063
unNorm = []
#for i in range(0, test_num_total):
for i in range(test_num_total*3, test_num_total*4):
#for i in range(0, test_num_total):
    xyz_test = xyz_test_list[i,:].astype(np.float32)
    where_are_nan = np.isnan(xyz_test)
    where_are_inf = np.isinf(xyz_test)
    xyz_test[where_are_nan] = 0
    xyz_test[where_are_inf] = 0
    nodes_pt_norm = xyz_test.reshape(14, 3) - xyz_test.reshape(14, 3)[13, :]
    unNorm.append(xyz_test.reshape(14, 3)[13, :])
    graph_tr.append({
        "globals": [1.0, -1.0],
        "nodes": nodes_pt_norm,
...
try:
  ...
  pass
sess = tf.Session()
restore_file = '/home/chen/Documents/GNN_demo/saved'+'/model.pickle'
...
# How much time between logging and printing the curre     q  nt results.
...
outputnode_unNorm = train_values["outputs"][0].nodes + train_values["input_graph"][0].nodes
for i in range(test_num_total):
    for j in range(14):
        outputnode_unNorm[j+14*i] = outputnode_unNorm[j+14*i] + unNorm[i]
np.savetxt("train_values_5_4.txt", outputnode_unNorm, fmt="%.5f")
np.savetxt("train_values_all_4.txt",outputnode_unNorm)  # 缺省按照'%.18e'格式保存数据，以空格分隔
print(train_values["outputs"][0].nodes)
"""
