...
class NyuDataset(BaseDataset):
    ...
    def loadAnnotation(self, is_trun=False):
        ...
        if self.subset == 'testing':
            with open('/home/chen/Documents/denseReg-master/data/nyu_bbx.pkl', 'rb') as f:
...
def saveTFRecord():
    ...
    #数据包括：图片（decode成png）,xyz坐标（从mat文件中读取，第二维坐标加个负号）存储在self.annotations中，
    #将原始数据拆分成300份用15个线程并行写入，用tf.train.Example组织，
    reader.write_TFRecord_multi_thread(num_threads=15, num_shards=300)
