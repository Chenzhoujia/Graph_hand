...
from network.chem_tensorflow import ChemModel
...
class SparseGGNNChemModel(ChemModel):
    ...
    def default_params(cls):
        params = dict(ChemModel.default_params())
        params.update({
            'batch_size': 100000,
            'use_edge_bias': False,
...
def main():
    args = {'--help':False, '--freeze-graph-model':False, '--config-file':None, '--data_dir':None, '--config':None, '--log_dir':None, '--restore':None}
    try:
